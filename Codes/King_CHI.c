...
int compress_bit(char *input1, char *input2, char *path_input2, char *output){
...
int file_check(char *input1, char *input2, char *output){
        int n_marker=0, n_marker1=0, n_marker2=0;

        FILE *fp_bim1=NULL, *fp_bim2=NULL, *fp_bim3=NULL, *fp_bim4=NULL, *fp_r=NULL;
    ...
    if((fp_r=fopen(fileName_temp,"w"))==NULL){
        ...
        struct gm a[2]={0};
        struct gm b[2]={0};
        while (fscanf(fp_bim1,"%s", a[0].chr)>0 && fscanf(fp_bim2,"%s", b[0].chr)>0) {
            if(strncmp(a[0].chr, b[0].chr, 6)==0){
                n_marker++;
            }else
                return -1;
        }
    ...
    /*output marker number of file1, file2, and overlapped*/
    strcpy (fileName_temp,input1);
    strcat (fileName_temp,".bim");
    if((fp_bim3=fopen(fileName_temp,"r"))==NULL){
        printf("The bim file of file1 doesn't exist!\n");
        return -1;
    }
    strcpy (fileName_temp,input2);
    strcat (fileName_temp,".bim");
    if((fp_bim4=fopen(fileName_temp,"r"))==NULL){
        printf("The bim file of file2 doesn't exist!\n");
        return -1;
    }
    ...
    struct bim f1[2]={0};
    struct bim f2[2]={0};
    while (fscanf(fp_bim3,"%s%s%s%s%s%s", f1[0].chr, f1[0].rs, f1[0].gen, f1[0].pos, f1[0].ref, f1[0].alt)>0) {
        n_marker1++;
    }
    while (fscanf(fp_bim4,"%s%s%s%s%s%s", f2[0].chr, f2[0].rs, f2[0].gen, f2[0].pos, f2[0].ref, f2[0].alt)>0) {
        n_marker2++;
    }
    printf("%d overlapped SNPs in two input files \n", n_marker-6);
    printf("%d overlapped SNPs in input file1 \n", n_marker1);
    printf("%d overlapped SNPs in input file2 \n", n_marker2);
    fprintf(fp_r, "input_file1_name\t%s\n", input1);
    fprintf(fp_r, "input_file2_name\t%s\n", input2);
    fprintf(fp_r, "number_markers_file1\t%d\n", n_marker1);
    fprintf(fp_r, "number_markers_file2\t%d\n", n_marker2);
    fprintf(fp_r, "number_markers_overlapped\t%d\n", n_marker);
    fclose(fp_bim1);
    fclose(fp_bim2);
    fclose(fp_bim3);
    fclose(fp_bim4);
...
int KING(char *input1, char *input2, char *output){
...
int KING_single(char *input1, char *input2, char *path_input2, char *output){
...
//    printf("%s\n",fileName_temp);
    ...
    strcat (fileName_temp,"bcftools");

    strcpy (fileName_temp,path_input1);
    strcat (fileName_temp,"bcftools merge --force-samples -f PASS,.,RefCall -g - -R ");
    ...
    strcat (fileName_temp," -Ou | bcftools filter -g 5 -e 'FORMAT/DP<1' |  bcftools view -v snps -f PASS -Ov | python3 ");
//    strcat (fileName_temp," -Ou | bcftools filter -g 5 -e \"FORMAT/DP<1\" |  bcftools view -v snps -f PASS -Ov | python3 "); // for windows
    strcat (fileName_temp,path_input1);
    strcat (fileName_temp,"filterVCF.py -x 0.99 -n 15 -p 50 > ");
    strcat (fileName_temp,temp_folder);
    strcat (fileName_temp,getFileName(fileName_input1));
    strcat (fileName_temp,getFileName(fileName_input2));
    strcat (fileName_temp,".vcf");
    ...
    system(fileName_temp);
    fprintf(fp_r, "%s\n", fileName_temp);
//    printf("bcftools%d\n",execv("/home/meng/KING/bin/bcftools",input2));
    
//    strcpy (fileName_temp,path_input1);
//    strcat (fileName_temp,"bcftools index -f ");
//    strcat (fileName_temp,temp_folder);
//    strcat (fileName_temp,getFileName(fileName_input1));
//    strcat (fileName_temp,getFileName(fileName_input2));
//    strcat (fileName_temp,".bcf");
//    strcat (fileName_temp," -o ");
//    strcat (fileName_temp,temp_folder);
//    strcat (fileName_temp,getFileName(fileName_input1));
//    strcat (fileName_temp,getFileName(fileName_input2));
//    strcat (fileName_temp,".bcf.csi");
...
//    
//    strcpy (fileName_temp,path_input1);
//    strcat (fileName_temp,"bcftools view -m2 -v snps -e 'FMT/DP<20||FMT/DP>200||(PL!=\".\"&&(smpl_sum(FMT/PL[*:])-smpl_max(FMT/PL[*:]))<50)' ");
////    strcat (fileName_temp,"bcftools view -m2 -v snps -e \"FMT/DP<20||FMT/DP>200||(PL!='.'&&(smpl_sum(FMT/PL[*:])-smpl_max(FMT/PL[*:]))<50)\" ");  //for windows
//    strcat (fileName_temp,temp_folder);
//    strcat (fileName_temp,getFileName(fileName_input1));
//    strcat (fileName_temp,getFileName(fileName_input2));
//    strcat (fileName_temp,".bcf");
//    strcat (fileName_temp," -Oz -o ");
//    strcat (fileName_temp,temp_folder);
//    strcat (fileName_temp,getFileName(fileName_input1));
//    strcat (fileName_temp,getFileName(fileName_input2));
//    strcat (fileName_temp,".vcf.gz");
//    printf("%s\n",fileName_temp);
//    system(fileName_temp);
//    fprintf(fp_r, "%s\n", fileName_temp);
...
//    printf("%s\n",fileName_temp);
    ...
    strcat (fileName_temp,getFileName(fileName_input1));
    strcat (fileName_temp,getFileName(fileName_input2));
    strcat (fileName_temp," --export A --out ");
    strcat (fileName_temp,temp_folder);
    strcat (fileName_temp,getFileName(fileName_input1));
    strcat (fileName_temp,getFileName(fileName_input2));
    strcat (fileName_temp," >> ");
...
//    printf("%s\n",fileName_temp);
    ...
    if (compress_bit(fileName_input1, fileName_input2, temp_folder, fileName_output)!=0) {
...
int main(int argc, char * argv[]) {
    char *fileName_input1 = "13047_50.GRCh38.md.bsqr.bcftools.bcf";//target file name
    char *fileName_input2 = "13047_3000.GRCh38.md.bsqr.bcftools.bcf";//data base file name
    char *path_input1 = "./";//pathway of app
    char *path_input2 = "./";//pathway of temp files
    char *fileName_output = "myKING";
    char fileName_temp[1000];
    int i=0, type=1, run=1;
    FILE *fp_r=NULL;
    char *s1, *s2, *s3, *s4, *s5, *s6, *s7;
    s1 = "--file";
    s2 = "--data";
    s3 = "--out";
    s4 = "--format";// type=1 means bcf; type=2 means vcf;
    s5 = "--module";//run=1 means loop the file list one by one; run=2 means conduct two input file only;
    s6 = "--path_app";
    s7 = "--path_temp";
...
//            printf(" --path_temp %s\n", path_input2);
    ...
    if (run == 2){
        ...
        if (multiple_run(fileName_input1, fileName_input2, path_input1, path_input2, type, fileName_output)!=0) {
            printf("\nThe King coefficient calculation can't be finished!\n");
            return -1;
        }
    }
